...
def run_all_heuristics(text: str, source_url: str = "") -> HeuristicResult:
    ...
    if compression > 0.72:  # Very compressible = formulaic (normal text is ~0.6-0.7)
        ...
        result.signals.append(HeuristicSignal(
            ...
            confidence=round(conf, 2),
            evidence=f"Compression ratio: {compression:.1%} (formulaic pattern)",
            detail="Text is more compressible than typical journalism"
        ))

    # 9. Attention gradient (emotional front-loading)
    gradient = score_attention_gradient(text)
    if gradient > 0.5:  # Strong front-loading
        conf = min(gradient / 1.5, 1.0)
        result.signals.append(HeuristicSignal(
            model="attention_gradient", technique="emotional_framing",
            confidence=round(conf, 2),
            evidence=f"Attention gradient: {gradient:.2f} (emotional front-loading)",
            detail="Emotional language concentrated in opening, evidence buried later"
        ))
...
# ============================================
# 8. Semantic compression (Kolmogorov complexity approximation)
# ============================================

def score_semantic_compression(text: str) -> float:
    """Approximate Kolmogorov complexity via gzip compression ratio.
    Propaganda is more compressible than journalism because it follows
    templates. Returns compression ratio (0.0-1.0, higher = more compressible = more formulaic).
    """
    import gzip
    if len(text) < 50:
        return 0.0
    ...
    ratio = 1.0 - (len(compressed) / len(text_bytes))
    return round(ratio, 4)


# ============================================
# 9. Attention gradient (emotional front-loading detection)
# ============================================

def score_attention_gradient(text: str) -> float:
    """Detect emotional front-loading: manipulative content puts emotional
    hooks in the first third and buries contradicting evidence later.
    Score = (emotion[first_third] - emotion[last_third]) / mean_emotion.
    Positive = front-loaded. Negative = back-loaded. Zero = even.
    """
    words = re.findall(r'\b\w+\b', text.lower())
    if len(words) < 30:
        return 0.0
