...
export default function ThreatScore({ data }: { data: any }) {
  ...
  // Score based on max confidence (not technique count) so Brief ≈ Detailed
  const maxConf = data.prism?.techniques?.reduce(
    (max: number, t: any) => Math.max(max, t.confidence || 0), 0
  ) || 0;

  let score = 0;
  score += techs > 0 ? Math.round(maxConf * 40) : 0;   // Up to 40 from highest confidence
  score += Math.min(fcs * 4, 30);                        // Up to 30 from fact-checks
  score += Math.round(tox * 20);                         // Up to 20 from toxicity
  score += fcs >= 3 ? 10 : 0;                            // Bonus if 3+ independent fact-checks
