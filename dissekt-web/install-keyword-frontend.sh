...
# SELF-CONTAINED: writes KeywordAnalysis.tsx + adds the 'Keyword topic' tab to Analyze
set -e
...
COMPONENT_EOF
echo "✅ src/components/KeywordAnalysis.tsx written"

# 2. Wire into analyze page
python3 << 'PYEOF'
...
# add the Keyword tab button after the Bulk button (before Compare)
old_btn = '''              <button onClick={() => { window.location.href = isInvited ? '/compare' : '/signup'; }}
...
new_btn = '''              <button onClick={() => setScanTab('keyword')}
...
print("✅ Analyze page now has the Keyword topic tab")
PYEOF

echo ""
echo "Verify:"
grep -n "KeywordAnalysis\|'keyword'\|Keyword topic" src/app/analyze/page.tsx | head
